          ...
          <div className="flex items-center justify-between">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent
                  side="left"
                  className="max-w-sm max-h-96 overflow-y-auto"
                >
                  <div className="p-3 space-y-3">
                    <div className="bg-blue-50 p-2 rounded">
                      <h4
                        className={`font-semibold text-blue-900 text-sm mb-1 ${isRTL ? "text-right" : ""}`}
                      >
                        {language === "en" ? "Premium Details" : "تفاصيل القسط"}
                      </h4>
                      <div className="text-xs text-blue-800 space-y-1">
                    ...
                    <div className="bg-green-50 p-2 rounded">
                      <h4
                        className={`font-semibold text-green-900 text-sm mb-1 ${isRTL ? "text-right" : ""}`}
                      >
                        {language === "en"
                          ? "Key Features"
                          : "المزايا الرئيسية"}
                      ...
                      <ul className="text-xs text-green-800 space-y-1">
                        {offer.features[language]
                          .slice(0, 3)
                          .map((feature, index) => (
                            <li
                              key={index}
                              className={`flex items-start gap-1 ${isRTL ? "flex-row-reverse" : ""}`}
                            >
                              <CheckCircle className="h-2 w-2 text-green-600 mt-0.5 flex-shrink-0" />
                              <span className={isRTL ? "text-right" : ""}>
                                {feature}
                    ...
                    <div className="bg-orange-50 p-2 rounded">
                      <h4
                        className={`font-semibold text-orange-900 text-sm mb-1 ${isRTL ? "text-right" : ""}`}
                      >
                        {language === "en"
                          ? "Important Terms"
                          : "الشروط المهمة"}
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center bg-white rounded-lg p-4 border">
            ...
            <div className="flex justify-center gap-2 mt-2 flex-wrap">
              <Badge variant="secondary" className="text-xs">
                {(offer.premiumRate * 100).toFixed(2)}%{" "}
                {language === "en" ? "rate" : "معدل"}
              </Badge>
              <Badge variant="outline" className="text-xs">
                {Math.round(offer.annualPremium / 12).toLocaleString()}{" "}
                {language === "en" ? "EGP/month" : "جنيه/شهر"}
              </Badge>
            </div>
          </div>
